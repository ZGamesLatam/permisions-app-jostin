...
import {
  DEFAULT_SEARCH_NG_SELECT_PAGINATION,
...
export class SearchNgSelectService<T> implements OnDestroy {
  ...
  public searchTerm(term: string): void {
    const dataLengthAvailable = this.data$.value.length < this.totalWithoutTerm;
    const termLengthAvailable = term.length >= this.lengthToSearch;
    if (dataLengthAvailable && termLengthAvailable) {
      const termExists = this.data$.value.some((item) =>
        item.label.toLowerCase().includes(term.toLowerCase())
      );
      if (!termExists) {
        this.searchTermTrigger.next(term);
      } else {
        this.filterWithTerm = this.initialFilter;
      }
    } else {
